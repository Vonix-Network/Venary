...
var PterodactylPage = {
    ...
    async render(container) {
        try {
            const r = await API.get('/api/pterodactyl/access/me');
    ...
    async _fetchServerInfo() {
        ...
        try {
            const info = await API.get('/api/pterodactyl/server-info?server=' + encodeURIComponent(this._serverId));
    ...
    async _fetchStatus() {
        if (!this._serverId) return;
        try {
            const r = await API.get('/api/pterodactyl/status?server=' + encodeURIComponent(this._serverId));
    ...
    async _fetchResources() {
        if (!this._serverId) return;
        try {
            const r = await API.get('/api/pterodactyl/resources?server=' + encodeURIComponent(this._serverId));
    ...
    async _sendCommand() {
        ...
        try {
            await API.post('/api/pterodactyl/command', { command: cmd, server: this._serverId });
        } catch (err) {
    ...
    async _power(action) {
        ...
        this._setBusy(true);
        try {
            await API.post('/api/pterodactyl/power', { action, server: this._serverId });
