...
window.DonationsAdminPage = {
    ...
    async renderOverview(area) {
        try {
            const stats = await API.get('/api/donations/admin/stats');
            area.innerHTML = `
    ...
    async renderRanks(area) {
        try {
            const ranks = await API.get('/api/donations/admin/ranks');
            let html = `
    ...
    async renderRankedUsers(area) {
        try {
            const data = await API.get('/api/donations/admin/ranked-users?limit=50');
            let html = `
    ...
    async renderHistory(area) {
        try {
            const [data, ranks] = await Promise.all([
                API.get('/api/donations/admin/donations?limit=50'),
                API.get('/api/donations/admin/ranks').catch(() => []),
            ]);
    ...
    async saveEditDonation(donationId) {
        ...
        try {
            await API.patch(`/api/donations/admin/donations/${donationId}`, {
    ...
    async saveRank(rankId) {
        ...
        try {
            if (rankId) {
                await API.put('/api/donations/admin/ranks/' + rankId, body);
                App.showToast('Rank updated!', 'success');
            } else {
                await API.post('/api/donations/admin/ranks', body);
    ...
    async deleteRank(rankId, name) {
        ...
        try {
            await API.delete('/api/donations/admin/ranks/' + rankId);
    ...
    async showGrantRankModal() {
        let ranks;
        try { ranks = await API.get('/api/donations/admin/ranks'); } catch { ranks = []; }
    ...
    async grantRank() {
        ...
        try {
            await API.post('/api/donations/admin/grant-rank', { user_id: userId, rank_id: rankId, duration_days: days });
    ...
    async addManualDonation() {
        ...
        try {
            await API.post('/api/donations/admin/manual-donation', payload);
    ...
    async renderCryptoSettings(area) {
        try {
            const [cfg, status, provCfg] = await Promise.all([
                API.get('/api/donations/admin/crypto/config'),
                API.get('/api/donations/admin/crypto/status').catch(() => ({})),
                API.get('/api/donations/admin/crypto/provider/config').catch(() => ({ active_provider: 'manual', providers: [], config: {} })),
            ]);
    ...
    async _toggleCryptoMasterSwitch(enable) {
        ...
        try {
            await API.put('/api/donations/admin/crypto/config', { payments_enabled: enable });
    ...
    async saveCryptoConfig() {
        try {
            ...
            await API.put('/api/donations/admin/crypto/config', body);
    ...
    async saveWalletSeeds() {
        ...
        try {
            await API.put('/api/donations/admin/crypto/wallet', body);
    ...
    async generateNewSeed() {
        ...
        try {
            const data = await API.post('/api/donations/admin/crypto/generate-seed', {});
    ...
    async _applyPendingSeed() {
        ...
        try {
            await API.put('/api/donations/admin/crypto/wallet', {
    ...
    async revealSeed() {
        ...
        try {
            const data = await API.get('/api/donations/admin/crypto/wallet/reveal');
    ...
    async renderBalanceSettings(area) {
        try {
            const cfg = await API.get('/api/donations/admin/crypto/config');
    ...
    async saveBalanceSettings() {
        ...
        try {
            await API.put('/api/donations/admin/crypto/config', { balance_display_currencies: checked });
    ...
    async renderUserBalances(area) {
        try {
            ...
            const data = await API.get(`/api/donations/admin/balances?search=${encodeURIComponent(search)}&limit=50`);
    ...
    _cuLookupDebounce() {
        ...
        this._cuTimer = setTimeout(async () => {
            ...
            try {
                ...
                const bal = await API.get(`/api/donations/admin/balances/${match.id}/ledger`).catch(() => null);
    ...
    async submitCreditUser() {
        ...
        try {
            const result = await API.post('/api/donations/admin/balances/adjust-by-username', { username, amount, reason });
    ...
    async submitAdjust(userId) {
        ...
        if (!reason) { App.showToast('Reason is required', 'warning'); return; }
        try {
            const result = await API.post(`/api/donations/admin/balances/${userId}/adjust`, { amount, reason });
    ...
    async showLedgerModal(userId, username) {
        try {
            const ledger = await API.get(`/api/donations/admin/balances/${userId}/ledger`);
    ...
    async renderWallet(area) {
        ...
        try {
            const { user_addresses, admin_addresses } = await API.get('/api/donations/admin/crypto/wallet/addresses');
    ...
    async _loadAllWalletBalances() {
        ...
        const fetchBalance = async (coin, address, idx) => {
            ...
            try {
                const data = await API.get(`/api/donations/admin/crypto/wallet/address/${coin}/${address}/balance`);
    ...
    async deriveWalletAddress() {
        ...
        try {
            const data = await API.post('/api/donations/admin/crypto/wallet/derive', { label: label.trim() });
    ...
    async showWalletTxModal(coin, address) {
        ...
        try {
            const data = await API.get(`/api/donations/admin/crypto/wallet/address/${coin}/${address}/transactions`);
    ...
    async showVerifyReport() {
        ...
        try {
            const data = await API.get('/api/donations/admin/crypto/wallet/verify');
    ...
    async cleanMalformedAddresses() {
        ...
        if (!confirmed) return;
        try {
            const data = await API.delete('/api/donations/admin/crypto/wallet/admin-addresses/malformed');
    ...
    async _renderPaymentProviders_unused(area) {
        try {
            const cfg = await API.get('/api/donations/admin/crypto/provider/config');
    ...
    _renderProviderConfigPanel(providerId, config, providers) {
        ...
        const webhookUrl = `${window.location.origin}/api/donations/crypto/webhook/${providerId}`;
    ...
    _selectProvider(id) {
        ...
        if (panel) {
            API.get('/api/donations/admin/crypto/provider/config').then(cfg => {
    ...
    async _loadCoinsGrid(providerId, enabledCoins) {
        ...
        let coins;
        try {
            const data = await API.get('/api/donations/crypto/currencies');
    ...
    async _testProviderConnection() {
        ...
        try {
            const result = await API.get(`/api/donations/admin/crypto/provider/test/${id}`);
            if (result.ok) {
    ...
    async _saveProviderSettings() {
        ...
        try {
            await Promise.all([
                API.put('/api/donations/admin/crypto/provider/config', payload),
                API.put('/api/donations/admin/crypto/config', {
                    enabled_coins:   enabledCoins,
    ...
    async _loadProviderDashboard() {
        ...
        try {
            const data = await API.get('/api/donations/admin/crypto/provider/dashboard');
