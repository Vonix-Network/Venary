/**
 * Venary — Forum Routes
 * Migrated from extensions/forum/server/routes.js
...
// Table aliases — prefixed to coexist with core 'posts' and messenger 'categories'
const T = {
...
router.post('/categories', authenticateToken, async (req, res) => {
    try {
        ...
        await db.run(
            `INSERT INTO ${T.categories} (id, name, description, icon, sort_order, created_at) VALUES (?, ?, ?, ?, ?, ?)`,
...
router.post('/categories/:id/threads', authenticateToken, async (req, res) => {
    try {
        ...
        const author = await db.get('SELECT username, display_name FROM users WHERE id = ?', [req.user.id]);
        res.status(201).json({
...
router.get('/threads/:id', async (req, res) => {
    try {
        ...
        const author = await db.get('SELECT username, display_name FROM users WHERE id = ?', [thread.user_id]);
...
router.post('/threads/:id/posts', authenticateToken, async (req, res) => {
    try {
        ...
        await db.run(
            `INSERT INTO ${T.posts} (id, thread_id, user_id, content, is_op, created_at, media) VALUES (?, ?, ?, ?, 0, ?, ?)`,
        ...
        const author = await db.get('SELECT username, display_name, level, role FROM users WHERE id = ?', [req.user.id]);
        res.status(201).json({
            id: postId,
            content,
            media,
