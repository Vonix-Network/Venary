...
module.exports = function (getNs) {
    ...
    router.post('/spaces/:spaceId/roles', authenticateToken, async (req, res) => {
        try {
            ...
            const ns = getNs(); if (ns) ns.to(`space:${req.params.spaceId}`).emit('role:created', role);
    ...
    router.put('/roles/:id', authenticateToken, async (req, res) => {
        try {
            ...
            const ns = getNs(); if (ns) ns.to(`space:${role.space_id}`).emit('role:updated', updated);
            res.json(updated);
        } catch (err) {
    ...
    router.delete('/roles/:id', authenticateToken, async (req, res) => {
        try {
            ...
            const ns = getNs(); if (ns) ns.to(`space:${role.space_id}`).emit('role:deleted', {
    ...
    // POST /members/:memberId/roles/:roleId — Assign
    router.post('/members/:memberId/roles/:roleId', authenticateToken, async (req, res) => {
        try {
            ...
            const role = await db.get('SELECT * FROM roles WHERE id = ? AND space_id = ?', [req.params.roleId, member.space_id]);
            if (!role) return res.status(404).json({ error: 'Role not found in this space' });

            await db.run('INSERT OR IGNORE INTO member_roles (member_id, role_id) VALUES (?, ?)', [req.params.memberId, req.params.roleId]);
            const ns = getNs(); if (ns) ns.to(`space:${member.space_id}`).emit('member:role_added', {
    ...
    // DELETE /members/:memberId/roles/:roleId — Remove
    router.delete('/members/:memberId/roles/:roleId', authenticateToken, async (req, res) => {
        try {
            ...
            await db.run('DELETE FROM member_roles WHERE member_id = ? AND role_id = ?', [req.params.memberId, req.params.roleId]);
            const ns = getNs(); if (ns) ns.to(`space:${member.space_id}`).emit('member:role_removed', {
                memberId: req.params.memberId, roleId: req.params.roleId, userId: member.user_id,
            });
            res.json({ success: true });
        } catch (err) {
