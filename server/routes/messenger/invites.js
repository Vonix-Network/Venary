...
module.exports = function (getNs) {
    ...
    router.post('/spaces/:spaceId/invites', authenticateToken, async (req, res) => {
        try {
            ...
            const expiresAt = (max_age && max_age > 0) ? new Date(Date.now() + max_age * 1000).toISOString() : null;
    ...
    router.post('/invites/:code/use', authenticateToken, async (req, res) => {
        try {
            ...
            const ban = await db.get('SELECT 1 FROM space_bans WHERE space_id = ? AND user_id = ?', [invite.space_id, req.user.id]);
            if (ban) return res.status(403).json({ error: 'You are banned from this space' });

            const existing = await db.get('SELECT 1 FROM members WHERE space_id = ? AND user_id = ?', [invite.space_id, req.user.id]);
            if (!existing) {
                const now = new Date().toISOString();
                await db.run('INSERT INTO members (id, space_id, user_id, joined_at) VALUES (?, ?, ?, ?)',
                    [uuidv4(), invite.space_id, req.user.id, now]);
                await db.run('UPDATE spaces SET member_count = member_count + 1 WHERE id = ?', [invite.space_id]);
                await db.run('UPDATE invites SET uses = uses + 1 WHERE code = ?', [req.params.code]);

                const ns = getNs();
                if (ns) ns.to(`space:${invite.space_id}`).emit('member:joined', { spaceId: invite.space_id, userId: req.user.id });
            }
