...
module.exports = function (getNs) { // eslint-disable-line no-unused-vars
        ...
        try {
            ...
            await db.run(
                `INSERT INTO bots (id, name, description, avatar, owner_id, token, is_public, created_at)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
                [botId, name, description || null, avatar || null, req.user.id, token, is_public ? 1 : 0, now]
            );
    ...
    router.get('/bots', authenticateToken, async (req, res) => {
        try {
            const bots = await db.all('SELECT * FROM bots WHERE owner_id = ? ORDER BY created_at DESC', [req.user.id]);
            res.json(bots);
        } catch (err) {
    ...
    router.put('/bots/:id', authenticateToken, async (req, res) => {
        try {
            ...
            const { name, description, avatar, is_public } = req.body;
            await db.run(
                `UPDATE bots SET name = COALESCE(?, name), description = COALESCE(?, description),
                 avatar = COALESCE(?, avatar), is_public = COALESCE(?, is_public) WHERE id = ?`,
                [name || null, description || null, avatar || null,
                 is_public !== undefined ? (is_public ? 1 : 0) : null, req.params.id]
            );

            res.json(await db.get('SELECT * FROM bots WHERE id = ?', [req.params.id]));
        } catch (err) {
    ...
    // POST /spaces/:spaceId/bots/:botId — Install
    router.post('/spaces/:spaceId/bots/:botId', authenticateToken, async (req, res) => {
        try {
            ...
            const bot = await db.get(
                'SELECT * FROM bots WHERE id = ? AND (owner_id = ? OR is_public = 1)',
            ...
            const now = new Date().toISOString();
            await db.run(
                `INSERT INTO bot_installations (bot_id, space_id, installed_by, installed_at) VALUES (?, ?, ?, ?)
    ...
    // DELETE /spaces/:spaceId/bots/:botId — Remove
    router.delete('/spaces/:spaceId/bots/:botId', authenticateToken, async (req, res) => {
        try {
            ...
            await db.run('DELETE FROM bot_installations WHERE bot_id = ? AND space_id = ?', [req.params.botId, req.params.spaceId]);
            res.json({ success: true });
        } catch (err) {
