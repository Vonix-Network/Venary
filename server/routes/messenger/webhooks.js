...
module.exports = function (getNs) {
        ...
        try {
            ...
            await db.run(
                `INSERT INTO webhooks (id, space_id, channel_id, name, avatar, token, created_by, created_at)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
                [webhookId, channel.space_id, req.params.channelId, name, avatar || null, token, req.user.id, now]
            );
    ...
    router.get('/channels/:channelId/webhooks', authenticateToken, async (req, res) => {
        try {
            ...
            const webhooks = await db.all('SELECT * FROM webhooks WHERE channel_id = ?', [req.params.channelId]);
            res.json(webhooks);
        } catch (err) {
        ...
        try {
            ...
            await db.run(
                `INSERT INTO channel_messages
                 ...
                 VALUES (?, ?, ?, ?, 'webhook', ?, ?)`,
                [msgId, webhook.channel_id, `webhook:${webhook.id}`, content || null, JSON.stringify(embeds || []), now]
            );
            await db.run('UPDATE channels SET last_message_at = ? WHERE id = ?', [now, webhook.channel_id]);
            ...
            const ns = getNs(); if (ns) ns.to(`channel:${webhook.channel_id}`).emit('channel:message', enriched);
