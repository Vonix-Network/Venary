/**
 * Venary — Donations Discord Integration
 * Migrated from extensions/donations/server/discord.js
...
module.exports = (discordBot, db) => {
    discordBot.registerCommand({
        ...
        try {
            const ranks = await db.all('SELECT name, price, description, icon FROM donation_ranks WHERE active = 1 ORDER BY sort_order ASC');
