/* =======================================
   ...
   Migrated from extensions/donations/server/crypto/monitor.js
   Now uses shared db and imported balanceMgr instead of params.
   ======================================= */
'use strict';

const { v4: uuidv4 } = require('uuid');
const db = require('../../db');
const balanceMgr = require('./balance');
...
async function pollSolanaIntents(Config) {
        ...
        await _withBackoff(async () => {
            ...
            for (const sigInfo of sigs) {
                ...
                if (confirmations >= SOL_CONFIRMATIONS) {
                    await completeCryptoIntent(intent.id, sigInfo.signature, receivedSol, Config);
                } else {
                    await db.run(
...
async function pollLitecoinIntents(Config) {
    const now = new Date().toISOString();
        ...
        await _withBackoff(async () => {
            ...
            for (const tx of txs) {
                ...
                if (confirmations >= LTC_CONFIRMATIONS) {
                    await completeCryptoIntent(intent.id, tx.hash, received, Config);
                } else {
                    await db.run(
                        `UPDATE crypto_payment_intents SET confirmations = ? WHERE id = ?`,
                        [confirmations, intent.id]
...
async function pollAnytimeAddresses(Config) {
    const addresses = await db.all('SELECT * FROM user_crypto_addresses');
    ...
    for (const row of addresses) {
        if (solEnabled && row.sol_address) {
            await _withBackoff(async () => {
                await _checkSolanaAnytime(row.user_id, row.sol_address, Config);
            }, `sol-anytime-${row.user_id}`);
        }

        if (ltcEnabled && row.ltc_address) {
            await _withBackoff(async () => {
                await _checkLitecoinAnytime(row.user_id, row.ltc_address);
            }, `ltc-anytime-${row.user_id}`);
        }
    }
}

async function _checkSolanaAnytime(userId, address, Config) {
    const rpcUrl = Config.get('donations.crypto.solana_rpc_primary', 'https://api.mainnet-beta.solana.com');
    const { Connection, PublicKey } = getSolanaWeb3();
    const connection = new Connection(rpcUrl, 'confirmed');
    ...
    for (const sigInfo of sigs) {
        ...
        const existing = await db.get('SELECT id FROM anytime_address_txs WHERE tx_hash = ?', [sigInfo.signature]);
        ...
        const receivedSol = receivedLamports / 1e9;
        await processAnytimeTx(userId, sigInfo.signature, receivedSol, 'sol');
    }
}

async function _checkLitecoinAnytime(userId, address) {
    ...
    for (const tx of (data.txs || [])) {
        ...
        const existing = await db.get('SELECT id FROM anytime_address_txs WHERE tx_hash = ?', [tx.hash]);
        if (existing) continue;
        ...
        if (received <= 0) continue;
        await processAnytimeTx(userId, tx.hash, received, 'ltc');
    }
}
 ...
 */
async function completeCryptoIntent(intentId, txHash, confirmedAmount, Config) {
    ...
    const intent = await db.get('SELECT * FROM crypto_payment_intents WHERE id = ?', [intentId]);
    if (!intent) return;
    ...
    try {
        ...
        if (discordWebhookUrl) {
            const rank = intent.rank_id
                ? await db.get('SELECT name, color, icon FROM donation_ranks WHERE id = ?', [intent.rank_id])
                : null;
            const user = intent.user_id
                ? await db.get('SELECT username, display_name FROM users WHERE id = ?', [intent.user_id])
                : null;
 ...
 */
async function processAnytimeTx(userId, txHash, cryptoAmount, coin) {
    let usdAmount = 0;
    let exchangeRate = 0;
    try {
        const rates = await exchange.getRates();
...
async function _grantRank(userId, rankId) {
    const rank = await db.get('SELECT * FROM donation_ranks WHERE id = ?', [rankId]);
    if (!rank) return;
...
function startMonitoring(Config) {
    ...
    if (solEnabled) {
        const h1 = setInterval(() => pollSolanaIntents(Config).catch(e =>
    ...
    if (ltcEnabled) {
        const h2 = setInterval(() => pollLitecoinIntents(Config).catch(e =>
            console.error('[Donations/Crypto] LTC intent poll error:', e.message)
        ), LTC_INTENT_INTERVAL_MS);
        _intervals.push(h2);
    }

    const h3 = setInterval(() => pollAnytimeAddresses(Config).catch(e =>
        console.error('[Donations/Crypto] Anytime poll error:', e.message)
    ), ANYTIME_INTERVAL_MS);
    _intervals.push(h3);

    setTimeout(() => pollAnytimeAddresses(Config).catch(() => {}), 30000);
