/* =======================================
   ...
   Live console, power controls, status badge.
   ======================================= */
var PterodactylPage = {
    _socket: null,
    _autoScroll: true,
    ...
    async render(container) {
        // Access check — render denied state early if no access
        let hasAccess = false;
        try {
            ...
            <div class="ptero-page animate-fade-up" style="max-width:1000px;margin:0 auto">
                ...
                <div class="ptero-controls animate-fade-up" style="animation-delay:0.05s">
                    ...
                    <div class="ptero-stop-group">
                        ...
                        <button id="ptero-btn-kill" class="btn ptero-btn-kill" onclick="PterodactylPage._power('kill')" title="Force kill the server process">
                ...
                <div class="admin-settings-card animate-fade-up" style="animation-delay:0.1s;padding:0;overflow:hidden">
                    <div class="ptero-console-toolbar">
                        <span style="font-size:0.75rem;color:var(--text-muted);font-family:var(--font-mono)">console output</span>
    ...
    async _fetchStatus() {
        try {
            const r = await API.get('/api/ext/pterodactyl-panel/status');
    ...
    _connectSocket() {
        ...
        // io() is provided by the Socket.IO client script loaded by the platform
        ...
        socket.on('disconnect', () => {
            this._showError('Disconnected from console stream.');
        });
    },
    ...
    _appendLine(line, scroll) {
        ...
        // Cap DOM lines at 500 to match server buffer
    ...
    async _power(action) {
        ...
        this._setBusy(true);
        try {
            await API.post('/api/ext/pterodactyl-panel/power', { action });
    ...
    _showError(msg) {
        ...
        // Disable power buttons when stream is broken
        this._setBusy(true);
    },
...
// Auto-scroll detection — set up once the console element exists
document.addEventListener('scroll', function (e) {
    const el = document.getElementById('ptero-console');
    if (!el || e.target !== el) return;
    const atBottom = el.scrollHeight - el.scrollTop - el.clientHeight < 40;
    PterodactylPage._autoScroll = atBottom;
}, true);
