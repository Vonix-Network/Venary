 ...
 * cfg: { provider, keyId, appKey, bucket, endpoint, publicUrl?, region? }
 */
 ...
 * cfg: { provider:'cloudinary', cloudName, apiKey, apiSecret }
 */
async function uploadCloudinary(cfg, buffer, filename, contentType) {
    ...
    // Sign: alphabetically sorted params
 ...
 * cfg: { provider:'bunny', accessKey, storageZone, region?, publicHostname? }
 * region: 'de' (default), 'ny', 'la', 'sg', 'syd'
 */
async function uploadBunny(cfg, buffer, filename, contentType) {
    ...
    // Public URL via pull zone hostname or default CDN
...
// =============================================================================
// Sequential Multi-Provider Upload
// Priority order: R2 → B2 → Cloudinary → Bunny
// Tries all keys of each provider before moving to the next.
// Hard config errors (400/404) stop immediately.
// =============================================================================
...
/**
 * Try all keys sequentially in priority order.
 * @param {Array}  allKeys     - raw key list from config
 * @param {Buffer} buffer
...
async function multiCloudUpload(allKeys, buffer, filename, contentType) {
    ...
    // Sort by provider priority: R2 → B2 → Cloudinary → Bunny → others
    ...
    for (let i = 0; i < ordered.length; i++) {
        ...
        try {
            ...
            // quota / auth / service error — try next
        }
    }
...
module.exports = (extDb) => {
    ...
    router.post('/upload', authenticateToken, uploadMiddleware, async (req, res) => {
        try {
            ...
            // 1. Multi-Cloud pool (R2 → B2 → Cloudinary → Bunny)
