...
/**
 * Check if a computed permission value includes a specific permission.
 */
 ...
 * @param {Object} db - Extension database instance
 * @param {string} spaceId
 * @param {string} userId
 * @param {string} ownerId - Space owner ID (always ADMINISTRATOR)
...
/**
 * Serialize a bigint permission value to a decimal string (for DB storage).
 */
function serializePerms(bigintVal) {
    return bigintVal.toString();
}

/**
 * Deserialize a decimal string from DB to bigint.
 */
function deserializePerms(str) {
    try { return BigInt(str || '0'); } catch { return 0n; }
}
