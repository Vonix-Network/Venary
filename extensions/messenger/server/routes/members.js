...
module.exports = function (db, ns) {
    ...
    router.put('/members/:id', authenticateToken, async (req, res) => {
        try {
            ...
            // Can set own nickname or MANAGE_SPACE can set anyone's
            ...
            if (ns) ns.to(`space:${member.space_id}`).emit('member:updated', {
    ...
    // POST /spaces/:spaceId/kick/:userId — Kick member
    router.post('/spaces/:spaceId/kick/:userId', authenticateToken, async (req, res) => {
        try {
            ...
            if (ns) {
                ...
                ns.to(`space:${req.params.spaceId}`).emit('member:left', {
                    spaceId: req.params.spaceId, userId: req.params.userId
                });
            }
    ...
    // POST /spaces/:spaceId/ban/:userId — Ban member
    router.post('/spaces/:spaceId/ban/:userId', authenticateToken, async (req, res) => {
        try {
            ...
            if (ns) {
                ns.to(`user:${req.params.userId}`).emit('member:banned', {
                    spaceId: req.params.spaceId, reason: reason || null
                });
                ns.to(`space:${req.params.spaceId}`).emit('member:left', {
                    spaceId: req.params.spaceId, userId: req.params.userId
                });
    ...
    // DELETE /spaces/:spaceId/ban/:userId — Unban member
