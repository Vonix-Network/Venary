/* =======================================
   Messenger — Spaces Routes
   ======================================= */
const express = require('express');
const { v4: uuidv4 } = require('uuid');
const { authenticateToken } = require('../../../../server/middleware/auth');
const { Permissions, DEFAULT_PERMISSIONS, computePermissions, hasPermission, serializePerms } = require('../permissions');
...
module.exports = function (db, ns) {
    ...
    router.post('/', authenticateToken, async (req, res) => {
        try {
            ...
            await db.run(
                ...
                [memberId, spaceId, ownerId, now]
            );

            const space = await db.get('SELECT * FROM spaces WHERE id = ?', [spaceId]);
            if (ns) ns.to(`user:${ownerId}`).emit('space:created', space);
    ...
    // GET /spaces — List spaces the user is a member of
    ...
    router.put('/:id', authenticateToken, async (req, res) => {
        try {
            ...
            if (ns) ns.to(`space:${req.params.id}`).emit('space:updated', updated);
    ...
    router.delete('/:id', authenticateToken, async (req, res) => {
        try {
            ...
            if (ns) ns.to(`space:${req.params.id}`).emit('space:deleted', { spaceId: req.params.id });
    ...
    router.post('/:id/join', authenticateToken, async (req, res) => {
        try {
            ...
            await db.run(
                'INSERT INTO members (id, space_id, user_id, joined_at) VALUES (?, ?, ?, ?)',
                [memberId, req.params.id, req.user.id, now]
            );
            await db.run(
            ...
            if (ns) {
                ns.to(`space:${req.params.id}`).emit('member:joined', {
                    spaceId: req.params.id, userId: req.user.id
                });
            }
    ...
    router.post('/:id/leave', authenticateToken, async (req, res) => {
        try {
            ...
            await db.run(
                'UPDATE spaces SET member_count = MAX(0, member_count - 1) WHERE id = ?',
                [req.params.id]
            );

            if (ns) {
                ns.to(`space:${req.params.id}`).emit('member:left', {
                    spaceId: req.params.id, userId: req.user.id
                });
            }
    ...
    // GET /spaces/public — Browse public spaces
