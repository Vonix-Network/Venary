...
module.exports = function (db, ns) {
    ...
    router.post('/spaces/:spaceId/channels', authenticateToken, async (req, res) => {
        try {
            ...
            if (ns) ns.to(`space:${req.params.spaceId}`).emit('channel:created', channel);
        ...
        try {
            ...
            await db.run(
                `UPDATE channels SET
                 ...
                 WHERE id = ?`,
                [name || null, topic || null, category_id || null, position != null ? position : null,
                 slowmode_seconds != null ? slowmode_seconds : null,
                 is_nsfw !== undefined ? (is_nsfw ? 1 : 0) : null, req.params.id]
            );

            const updated = await db.get('SELECT * FROM channels WHERE id = ?', [req.params.id]);
            if (ns) ns.to(`space:${channel.space_id}`).emit('channel:updated', updated);
    ...
    router.delete('/channels/:id', authenticateToken, async (req, res) => {
        try {
            ...
            if (ns) ns.to(`space:${channel.space_id}`).emit('channel:deleted', {
